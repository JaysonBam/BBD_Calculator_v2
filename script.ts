(() => {
    ...
    if (calculator){
        ...
        const operators: { [key: string]: { prec: number, assoc: 'left' | 'right' } } = {
            '^': { prec: 4, assoc: 'right' },
            '*': { prec: 3, assoc: 'left' },
            '/': { prec: 3, assoc: 'left' },
            '+': { prec: 2, assoc: 'left' },
        ...
        function buildExpressionTree(postfix: string): Node{
            const stack: Node[] = [];
        ...
        function evaluateTree(tree: Node): string{
            if (!tree) return '0';
        ...
        function calculate(infix: string): string{
            try {
